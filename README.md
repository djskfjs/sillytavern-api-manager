...
- 账户管理直接显示在酒馆的 **API 连接设置（插头图标）** 中，适配侧栏宽度。
...
4. 安装完成后重新加载页面，点击顶部 **插头图标（API 连接）**，在 API 类型选择框下找到“API 账户”。
...
1. 打开顶部 **插头图标 → API 账户**，在“添加 API”中同时填写账户名称和分组。分组可选，也可以从已有分组中选择。
...
从 1.0.0 更新后，请刷新页面，入口会从扩展设置移到 API 连接设置。已有账户、Key、分组和余额设置会保留。

## 余额接口说明

不同服务商的余额接口和返回字段并不统一。预设只提供已知的 URL/字段；如果服务商没有公开余额接口，账户会显示“未配置接口”。对于其他服务商，可在同一表单的“自定义余额接口”折叠区域内填写完整余额 URL 和字段路径。余额请求使用浏览器 `fetch`，如果服务商没有允许跨域（CORS），浏览器会拒绝请求；这不是扩展可以绕过的限制，可改用酒馆后端代理或配置服务商的 CORS。
